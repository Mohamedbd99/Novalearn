...
final class ExerciceController extends AbstractController
{
    ...
    #[Route('/exercice_cours{courseId}', name: 'app_exercice_by_course', methods: ['GET'])]
    public function indexByCourse(int $courseId, ExerciceRepository $exerciceRepository): Response
    {
        // Récupérer les exercices associés à un cours spécifique
        $exercices = $exerciceRepository->findBy(['course' => $courseId]);

        // Vérifier si des exercices sont trouvés pour ce cours
        if (empty($exercices)) {
            // Aucune exercice trouvé, afficher un message informatif
            return $this->render('exercice/no_exercices.html.twig', [
                'courseId' => $courseId,
            ]);
        }

        // Rendre la vue avec les exercices filtrés
        return $this->render('exercice/index.html.twig', [
            'exercices' => $exercices,
        ]);
    }
    ...
    #[Route('/exercice_new', name: 'app_exercice_new', methods: ['GET', 'POST'])]
    ...
    #[Route('/{id}_edit', name: 'app_exercice_edit', methods: ['GET', 'POST'])]
